...
void delete_elements(int*, int*, int);
...
int search_binary(int* array_ptr, int left, int right, const int key)
{
    int midd = 0;
    while (true)
    {
        midd = (left + right) / 2;
        int mid = array_ptr[midd] - 0;

        if (key < mid)       // если искомое меньше значения в ячейке
            right = midd - 1;      // смещаем правую границу поиска
        else if (key > mid)  // если искомое больше значения в ячейке
...
void delete_elements(int* array_ptr, int* second_array_ptr, const int length)
{
    for (auto counter = 0, second_counter = 0; counter < length; counter++)
    {
        if (counter <= 16)
        {
        ...
        {
            if ((counter <= 27 && counter % 2 == 0) || counter > 27)
            {
                second_array_ptr[second_counter] = array_ptr[counter];
                second_counter++;
            }
